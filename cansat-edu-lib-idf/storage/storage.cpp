...
bool init(spi_host_device_t host) {
    // NOTE: spi_bus_initialize(host, ...) must already have been called by
    // the caller — this bus is shared with the LoRa radio (separate CS line).
    ...
    esp_err_t ret = esp_vfs_fat_sdspi_mount("/sdcard", &sd_host, &slot_cfg, &mnt_cfg, &_card);
    if (ret != ESP_OK) {
